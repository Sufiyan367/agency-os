...
6. **Production Truth & Provenance Integrity**: The CEO Command Center and Live Pipeline enforce strict, zero-inference provenance boundaries. Deals strictly originate from explicit won commercial records (never inferred from payments); verified revenue requires confirmed, settled external payments on non-mock gateways (proposals and pending invoices never count as revenue); activity streams fail-closed (`UNKNOWN` fallback) ensuring unclassified or ambiguous operations never contaminate production monitoring feeds.
...
11. **Dynamic Multi-Niche Portfolio & Truthful Telemetry**: The autonomous market selection engine dynamically balances active target corridors across multiple high-value commercial sectors (Legal Services, Medical Clinics, Real Estate, Dental, HVAC, Remodeling, Plumbing) rather than collapsing into single-niche monopolies. It enforces strict portfolio distribution ceilings (no single niche > 30%, max 2 per niche, max 1 per city) and truthful Explore vs Exploit semantics: unproven markets or those with sample size $N < 10$ strictly remain in `EXPLORE` mode with explicit `INSUFFICIENT REAL DATA` indicators. Operational selection scores (prioritization rank) are strictly distinguished from financial revenue.
