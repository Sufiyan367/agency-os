...
export class DeliverabilityHUDController {
    ...
    public renderReadinessBadge(container: HTMLElement, report: DeliverabilityReadinessReport): void {
        const authStatus = (report as any).auth_status || report.outbound_authorization || 'BLOCKED';
        const isAuthBlocked = authStatus === 'BLOCKED';
        const authBlocker = (report as any).reason || report.outbound_auth_blocker;
        ...
        const sender = (report as any).address || report.sender || 'hello@automatedagencyos.tech';
