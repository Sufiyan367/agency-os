 ...
 * Language: TypeScript
 * Description: Client controller managing push subscriptions, notification drawer,
 *              live unread badges, multi-category filtering, and in-app alert toasts.
 */
...
export class NotificationCenterController {
  ...
  private async initServiceWorker() {
    ...
    try {
      ...
      // Listen for notification click message from service worker
  ...
  private updatePushUI(active: boolean, label: string) {
    const pushBtn = document.getElementById('btn-enable-push');
  ...
  public setFilter(category: string) {
    ...
    filterPills.forEach((pill) => {
      ...
      if ((p.dataset.category || 'ALL') === category) {
  ...
  public async loadNotifications() {
    ...
    try {
      ...
      const data: NotificationListResponse = await resp.json();
  ...
  private renderNotificationList(items: NotificationItem[]) {
    const listContainer = document.getElementById('notification-list-items');
    if (!listContainer) return;
    ...
    items.forEach((item) => {
      ...
      html += `
        <div class="notification-card ${isUnread ? 'unread' : 'read'}" data-id="${item.id}" style="${borderGlow} background: ${isUnread ? '#0f141d' : '#0a0d13'}; border-radius:8px; padding:12px 14px; margin-bottom:10px; transition: all 0.2s ease;">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; pt-1;">
            <div>
              ${item.deep_link ? `
  ...
  public async handleItemClick(notificationId: number, deepLink: string) {
  ...
  public async markRead(notificationId: number) {
    try {
      ...
      const card = document.querySelector(`.notification-card[data-id="${notificationId}"]`) as HTMLElement;
      if (card) {
  ...
  public async triggerTestEvent(eventType: string) {
    try {
      ...
      if (resp.ok) {
        ...
        this.showToast('Test Alert Dispatched', `${res.notification?.title || eventType}`, 'info');
        await this.fetchUnreadCount();
        if (this.drawerOpen) {
  ...
  public showToast(title: string, body: string, type: 'info' | 'critical' | 'success' = 'info') {
...
declare global {
  interface Window {
    notificationCenter: NotificationCenterController;
