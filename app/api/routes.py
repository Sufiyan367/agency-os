...
async def get_owner_attention_endpoint(
    ...
    Scans agency operations via AttentionEngine and enforces CEO Notification Policy.
    """
    from app.crm.attention_engine import attention_engine
    feed_data = await attention_engine.get_attention_feed(db)
    return {
        "items": feed_data["feed"],
        "count": feed_data["high_priority_count"],
        "high_priority_count": feed_data["high_priority_count"],
        "medium_priority_count": feed_data["medium_priority_count"],
        "normal_priority_count": feed_data["normal_priority_count"],
        "status": feed_data["status"],
        "ceo_notification_required": feed_data["ceo_notification_required"],
        "message": feed_data["message"],
        "details": feed_data["items"]
    }

@router.get("/api/crm/contacted-history")
async def get_contacted_history_endpoint(
    db: AsyncSession = Depends(get_db),
    user_info: Dict[str, str] = Depends(get_current_user_info)
):
    """
    Historical Visibility Endpoint (Requirement 7):
    Provides complete answers to:
    ...
    """
    from app.crm.attention_engine import attention_engine
    return await attention_engine.get_contacted_history(db)
